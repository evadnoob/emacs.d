...
;;; Hummingbird's HCLFTPD server, v5.1
;;;
...
(defconst efs-dos:hummingbird-file-line-regexp
  (concat
   "^"
   "[-a-z][-a-z][-a-z][-a-z][-a-z][-a-z][-a-z][-a-z][-a-z][-a-z] +"
   "[0-9]+ +[A-Za-z]+ +"		; some number and group
   "[0-9]+ +"				; size
   "\\(Jan\\|Feb\\|Mar\\|Apr\\|May\\|Jun\\|Jul\\|Aug\\|Sep\\|Oct\\|Nov\\|Dec\\) +"
   "[ 0-3][0-9] "			; day of the month
   "[12][0-9]:[0-6][0-9] "))		; time
...
(efs-defun efs-parse-listing dos
  ...
  (efs-save-match-data
	...
	(looking-at " [0-9]+ File(s)\n"))
     ...
     ;; Check for Hummingbird's HCLFTPD server
     ((re-search-forward efs-dos:hummingbird-file-line-regexp nil t)
      (efs-add-listing-type 'dos:hummingbird host user)
      (efs-parse-listing 'dos:hummingbird host user dir path switches))
...
(efs-defun efs-parse-listing dos:microsoft
  (host user dir path &optional switches)
  ;; Parse the current buffer which is assumed to be a Microsoft DOS FTP listing.
  ;; HOST = remote host name
  ;; USER = remote user name
  ;; DIR = directory as a full remote path
  ;; PATH = directory in full efs-path syntax
...
;;;;--------------------------------------
;;;; Hummingbird FTPD, essentially winsock
;;;;--------------------------------------

(defconst efs-dos:hummingbird-date-and-size-regexp
  (concat
   " \\([0-9]+\\) "
   "\\(Jan\\|Feb\\|Mar\\|Apr\\|May\\|Jun\\|Jul\\|Aug\\|Sep\\|Oct\\|Nov\\|"
   "Dec\\) [ 0-3][0-9] \\( [12][0-9][0-9][0-9]\\|[0-2][0-9]:[0-6][0-9]\\) +"))

(efs-defun efs-parse-listing dos:hummingbird
  (host user dir path &optional switches)
  ;; Parse the current buffer which is assumed to be an NT Hummingbird listing.
  ;; HOST = remote host name
  ;; USER = remote user name
  ;; DIR = directory as a full remote path
  ;; PATH = directory in full efs-path syntax
  ...
  (goto-char (point-min))
  (efs-save-match-data
    (if (re-search-forward efs-dos:hummingbird-date-and-size-regexp nil t)
	(let ((tbl (efs-make-hashtable))
	      size dirp)
	  (while
	      (progn
		...
		(efs-put-hash-entry
		 (buffer-substring (point) (progn (end-of-line) (point)))
		 (list dirp size) tbl)
		(re-search-forward efs-dos:hummingbird-date-and-size-regexp nil t)))
	  (efs-put-hash-entry "." '(t) tbl)
	  (efs-put-hash-entry ".." '(t) tbl)
	  tbl))))

(defconst efs-dired-dos:hummingbird-re-exe "\\.exe$")

(or (assq 'dos:hummingbird efs-dired-re-exe-alist)
    (setq efs-dired-re-exe-alist
	  (cons (cons 'dos:hummingbird  efs-dired-dos:hummingbird-re-exe)
 		efs-dired-re-exe-alist)))

(defconst efs-dired-dos:hummingbird-re-dir "^. +d")

(or (assq 'dos:hummingbird efs-dired-re-dir-alist)
    (setq efs-dired-re-dir-alist
 	  (cons (cons 'dos:hummingbird efs-dired-dos:hummingbird-re-dir)
 		efs-dired-re-dir-alist)))

(efs-defun efs-dired-insert-headerline dos:hummingbird (dir)
  (efs-dired-pc-insert-headerline dir))

(efs-defun efs-dired-manual-move-to-filename dos:hummingbird
  (&optional raise-error bol eol)
  ;; In dired, move to the first char of filename on this line.
  ;; Returns (point) or nil if raise-error is nil, and there is no
  ;; no filename on this line.
  ...
  (let (case-fold-search)
	...
	(goto-char bol)
      (skip-chars-backward "^\n\r")
      (setq bol (point)))
    (if (re-search-forward efs-dos:hummingbird-date-and-size-regexp eol t)
	(point)
      (if raise-error
	  (error "No file on this line")
	(goto-char bol)))))

(efs-defun efs-dired-manual-move-to-end-of-filename dos:hummingbird
  (&optional no-error bol eol)
  ;; Assumes point is at the beginning of filename.
  ;; So, it should be called only after (dired-move-to-filename t)
  ;; On failure signals an error, or returns nil.
...
(efs-defun efs-dired-fixup-listing dos:hummingbird
  (file path &optional switches wildcard)
  (sort-fields 8 (point-min) (point-max)))
